...
public class MarketDataDao implements CrudRepository<IexQuote, String> {
  ...
  /**
   * Constructor for the MarketDataDao, it takes a HttpClientConnection Manager and the
   * MarketDataConfig.
   *
  ...
  /**
   * Finds a single IexQuote given the unique ID/symbol/ticker in String format. Returns the
   * optional object which could either be empty or contain the IexQuote if found.
   *
   ...
   * @throws DataRetrievalFailureException Throws if the call to findAllById retrieves more than one
   *                                       quote meaning there is some unexpected logical error.
   */
  @Override
   ...
   * objects populated with data from each object in the JSONObject. calls executeHttpGet to get the
   * http response body calls mapJsonToIexQuote to get a List of IexQuotes and returns it.
   *
   * @param symbols Iterable String object containing unique IDs/symbols/tickers.
   * @return returns a List of IexQuote objects.
   * @throws IllegalArgumentException      throws if ID/symbol/ticker is invalid.
   * @throws DataRetrievalFailureException throws if unexpected error when retrieving data from
   *                                       API.
   */
  @Override
  public List<IexQuote> findAllById(Iterable<String> symbols) {
    ...
    if (iexQuotesJson.length() == 0 || iexQuotesJson.length() != counter) {
  ...
  private Optional<String> executeHttpGet(String url) {
    ...
    try (CloseableHttpClient httpClient = getHttpClient();
      ...
      if (responseCode == HttpStatus.SC_NOT_FOUND) {
        ...
        throw new DataRetrievalFailureException("Unexpected Error When Retrieving IEX Data: "
  ...
  /**
   * Given the JsonObject containing one or more quote objects in Json String format, and the String
   * iterator, for each symbol in the String iterator convert the appropriate JsonObject from the
   * key into a String, and use this String to populate the IexQuote.
   *
  ...
  private List<IexQuote> mapJsonToIexQuote(JSONObject jsonObject, Iterator<String> iterator) {
    ...
    while (iterator.hasNext()) {
      try {
        ...
        throw new IllegalArgumentException("Unexpected IEX Data Error: "
